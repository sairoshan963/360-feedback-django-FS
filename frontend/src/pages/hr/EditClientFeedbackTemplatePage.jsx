...
import { getClientFeedbackTemplate, updateClientFeedbackTemplate } from '../../api/clientFeedback';
...
export default function EditClientFeedbackTemplatePage() {
  usePageTitle('Edit Client Feedback Template');
  const { id } = useParams();
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await getClientFeedbackTemplate(id);
        const t = res.data?.template;
        if (!t) { message.error('Template not found'); navigate('/hr/client-feedback-templates'); return; }
        setName(t.name || '');
        setDescription(t.description || '');
        setSections((t.sections || []).map((sec) => ({
          key: sec.id || Date.now() + Math.random(),
          id:  sec.id,
          title: sec.title || '',
          questions: (sec.questions || []).map((q) => ({
            key:           q.id || Date.now() + Math.random(),
            id:            q.id,
            question_text: q.question_text || '',
            type:          q.type || 'RATING',
            applies_to:    q.applies_to || 'EACH_EMPLOYEE',
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateClientFeedbackTemplate(id, payload);
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card>
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          <Title level={4} style={{ margin: 0 }}>Edit Client Feedback Template</Title>
          <Space>
