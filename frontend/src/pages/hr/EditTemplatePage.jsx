...
import {
  ...
  InputNumber, Divider, message, Spin,
} from 'antd';
import { PlusOutlined, DeleteOutlined, MinusCircleOutlined } from '@ant-design/icons';
import { getTemplate, updateTemplate } from '../../api/cycles';
...
export default function EditTemplatePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!template) { message.error('Template not found'); navigate('/hr/templates'); return; }
        setName(template.name || '');
        setSections((template.sections || []).map((sec) => ({
          key: sec.id || Date.now() + Math.random(), id: sec.id, title: sec.title || '',
          questions: (sec.questions || []).map((q) => ({
            key: q.id || Date.now() + Math.random(), id: q.id,
            question_text: q.question_text || '', type: q.type || 'RATING',
            rating_scale_min: q.rating_scale_min || 1, rating_scale_max: q.rating_scale_max || 5,
            is_required: q.is_required !== false,
          })),
        })));
      } catch (err) {
        message.error(err.response?.data?.message || 'Failed to load template');
  ...
  const handleSave = async () => {
    ...
    for (const sec of sections) {
      ...
      for (const q of sec.questions) { if (!q.question_text.trim()) { message.error('Every question needs text'); return; } }
    }
    const payload = {
      name: name.trim(),
      sections: sections.map((sec, si) => ({
        id: sec.id, title: sec.title.trim(), display_order: si + 1,
        questions: sec.questions.map((q, qi) => ({
          id: q.id, question_text: q.question_text.trim(), type: q.type,
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Card>
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          <Title level={4} style={{ margin: 0 }}>Edit Feedback Template</Title>
          <Space>
            ...
            <Card key={q.key} size="small" style={{ marginBottom: 12, background: '#fafafa' }}
              ...
              <Form layout="vertical">
                <Form.Item label="Question Text" required style={{ marginBottom: 8 }}>
                  <Input.TextArea rows={2} value={q.question_text} onChange={(e) => updateQuestion(sec.key, q.key, 'question_text', e.target.value)} />
                </Form.Item>
                <Space wrap>
                  ...
                  {q.type === 'RATING' && (
                    <>
                      <Form.Item label="Min" style={{ marginBottom: 0 }}><InputNumber min={1} max={10} value={q.rating_scale_min} onChange={(v) => updateQuestion(sec.key, q.key, 'rating_scale_min', v)} style={{ width: 70 }} /></Form.Item>
                      <Form.Item label="Max" style={{ marginBottom: 0 }}><InputNumber min={1} max={10} value={q.rating_scale_max} onChange={(v) => updateQuestion(sec.key, q.key, 'rating_scale_max', v)} style={{ width: 70 }} /></Form.Item>
                    </>
                  )}
