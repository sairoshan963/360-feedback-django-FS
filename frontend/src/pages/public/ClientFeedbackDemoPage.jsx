/**
 * DEMO PAGE — preview all question types, one-person-at-a-time wizard.
 * Route: /client-feedback-demo  (public, no auth needed)
 * Delete this file once the real form UI is finalised.
 */
import { useState, useMemo } from 'react';
import { Rate, Input, Button, Typography, Space, Avatar, Tag, Progress } from 'antd';
import {
...
const PROJECT = 'Gamyam Digital Platform';
const CLIENT  = 'Rajesh Kumar';

const EMPLOYEES = [
  { id: 'e1', first_name: 'Arjun',  last_name: 'Kumar',  department: 'Engineering' },
  { id: 'e2', first_name: 'Priya',  last_name: 'Sharma', department: 'Design' },
  { id: 'e3', first_name: 'Rohan',  last_name: 'Mehta',  department: 'Backend' },
  { id: 'e4', first_name: 'Sneha',  last_name: 'Reddy',  department: 'QA' },
];

const EMP_QUESTIONS = [
  { id: 'q1', text: "How would you rate this person's overall performance?", type: 'RATING',           helper: 'Rate from 1 (poor) to 5 (excellent).', required: true },
  { id: 'q2', text: 'How effectively did this person communicate with you?', type: 'RATING_WITH_TEXT', helper: 'Add a comment to support your rating.',  required: true },
];

const TEAM_QUESTIONS = [
  { id: 'q3', text: 'Was the project delivered on time and within scope?',    type: 'YES_NO', helper: '',                                                required: true  },
  { id: 'q4', text: 'How likely are you to recommend our team to others?',    type: 'NPS',    helper: '0 = Not at all likely · 10 = Extremely likely.', required: true  },
  { id: 'q5', text: 'What did the team do well? What could be improved?',     type: 'TEXT',   helper: 'Be as specific as possible — it helps us grow.', required: false },
];
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 16 }}>
        <Text style={{ fontSize: 15, fontWeight: 600, color: '#111' }}>
          {qi + 1}. {q.text}
          {q.required && <span style={{ color: '#ff4d4f', marginLeft: 3 }}>*</span>}
        </Text>
        {q.helper && (
...
const TOTAL_STEPS = EMPLOYEES.length + (TEAM_QUESTIONS.length > 0 ? 1 : 0);

export default function ClientFeedbackDemoPage() {
  const [step,      setStep]      = useState(0);
  const [answers,   setAnswers]   = useState({});
  const [submitted, setSubmitted] = useState(false);

  const isTeamStep = step === EMPLOYEES.length;
  const emp        = !isTeamStep ? EMPLOYEES[step] : null;
  ...
  const pct = useMemo(() => {
    let done = 0, total = 0;
    EMPLOYEES.forEach((e) => EMP_QUESTIONS.filter((q) => q.required).forEach((q) => {
      total++;
      if (isAnswered(q, (answers[e.id] || {})[q.id])) done++;
    }));
    TEAM_QUESTIONS.filter((q) => q.required).forEach((q) => {
      total++;
  ...
  if (submitted) return (
    <div style={{ minHeight: '100vh', background: '#f5f7fa', display: 'flex', flexDirection: 'column' }}>
      <TopBar project={PROJECT} pct={pct} />
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 440 }}>
          ...
          <Title level={3} style={{ margin: '0 0 8px' }}>Thank you, {CLIENT}!</Title>
          <Paragraph type="secondary" style={{ fontSize: 15, margin: '0 0 12px' }}>
            Your feedback for <strong>{PROJECT}</strong> has been received.
            Gamyam appreciates the time you've taken to share your experience.
          </Paragraph>
          <Text type="secondary" style={{ fontSize: 12 }}>
            Your responses are confidential and visible only to Gamyam's project managers.
          </Text>
        </div>
      </div>
      <Footer />
      ...
      <div style={{ flex: 1, maxWidth: 640, width: '100%', margin: '0 auto', padding: '28px 16px 60px' }}>
        ...
        <div style={{ marginBottom: 28, textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
            {EMPLOYEES.map((e, i) => {
              const done    = EMP_QUESTIONS.every((q) => isAnswered(q, (answers[e.id] || {})[q.id]));
              ...
              return (
                <div key={e.id} onClick={() => setStep(i)} title={displayName(e)}
                  style={{
                    ...
                    background: done ? '#52c41a' : current ? '#1677ff' : '#d9d9d9',
                    cursor: 'pointer', transition: 'all .25s',
                  }}
                />
              );
            })}
            {TEAM_QUESTIONS.length > 0 && (
              <div onClick={() => setStep(EMPLOYEES.length)} title="Team Overall"
                style={{
                  width: isTeamStep ? 28 : 10, height: 10, borderRadius: 10,
                  background: isTeamStep ? '#1677ff'
                    : TEAM_QUESTIONS.every((q) => isAnswered(q, (answers.team || {})[q.id])) ? '#52c41a'
                    : '#d9d9d9',
                  cursor: 'pointer', transition: 'all .25s',
                }}
              />
          ...
          <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 6 }}>
            {isTeamStep
              ? `Team Overall  ·  ${step + 1} of ${TOTAL_STEPS}`
              : `Person ${step + 1} of ${EMPLOYEES.length}${TEAM_QUESTIONS.length > 0 ? '  ·  then Team Overall' : ''}`}
          </Text>
        </div>

        {/* content */}
        {!isTeamStep
          ? <PersonView emp={emp} step={step} questions={EMP_QUESTIONS} answers={answers} onChange={setEmpAns} />
          : <TeamView questions={TEAM_QUESTIONS} answers={answers} onChange={setTeamAns} />
        }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, alignItems: 'center' }}>
          ...
          <Space size={5}>
            {EMPLOYEES.map((e, i) => {
              ...
              return (
                <Avatar key={e.id} size={28} onClick={() => setStep(i)}
                  style={{
                    background:  current ? avatarColor(i) : done ? '#52c41a' : '#e8e8e8',
                    fontSize: 10, fontWeight: 700, cursor: 'pointer',
                    outline: current ? `3px solid ${avatarColor(i)}` : 'none',
            ...
            {TEAM_QUESTIONS.length > 0 && (
              <Avatar size={28} icon={<TeamOutlined style={{ fontSize: 12 }} />}
                onClick={() => setStep(EMPLOYEES.length)}
                style={{
                  background: isTeamStep ? '#1677ff'
                    : TEAM_QUESTIONS.every((q) => isAnswered(q, (answers.team || {})[q.id])) ? '#52c41a'
                    : '#e8e8e8',
                  cursor: 'pointer',
                  outline: isTeamStep ? '3px solid #1677ff' : 'none',
                  outlineOffset: 2, transition: 'all .2s',
                }}
              />
            )}
          ...
          {isTeamStep ? (
            <Button type="primary" size="large" icon={<SendOutlined />}
              disabled={!allDone} onClick={() => setSubmitted(true)}
              style={{
                borderRadius: 10, minWidth: 160, height: 44,
                boxShadow: allDone ? '0 4px 14px rgba(22,119,255,.4)' : 'none',
              }}>
              Submit Feedback
            </Button>
          ) : (
            <Button type="primary" size="large"
              ...
              {step === EMPLOYEES.length - 1
                ...
                : <>Next: {displayName(EMPLOYEES[step + 1])} <ArrowRightOutlined style={{ fontSize: 11, marginLeft: 4 }} /></>
              }
            </Button>
          )}
        </div>
